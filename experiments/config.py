...
class RedConfig:
    def __init__(self, **kwargs):
        ...
        #
        self._transformations = misc.get_default(kwargs, 'transformations')
        # Initial permutation.
        ...
        self.add_use_scale = misc.get_default(
            kwargs, 'add_use_scale', False)
        ...
        self.sort_dims = misc.get_default(
            kwargs, 'sort_dims', None)
        self.sort_transform = misc.get_default(
            kwargs, 'sort_transform', None)

        self.conditioning_net = misc.get_default(
            kwargs, 'conditioning_net', None)
        self.condition_params = misc.get_default(
            kwargs, 'condition_params', None)
        ...
        self.no_conditioning_set = misc.get_default(
            kwargs, 'no_conditioning_set', False)

        # Sampling Options.
        #
        self.sample_batch_size = misc.get_default(
            kwargs, 'sample_batch_size', 128)
        self.sample_batch_size_n = misc.get_default(
            kwargs, 'sample_batch_size_n', 100)
        self.nsample_batches = misc.get_default(kwargs, 'nsample_batches', 10)
        self.samp_per_cond = misc.get_default(kwargs, 'samp_per_cond', 1)
        ...
        self.embed_layers = misc.get_default(kwargs, 'embed_layers', 256)
        ...
        self.K_0 = misc.get_default(kwargs, 'K_0', 100)
        self.K = misc.get_default(kwargs, 'K', 5)
        self.use_ind_comp = misc.get_default(kwargs, 'use_ind_comp', False)
        self.conditional_embed = misc.get_default(
            kwargs, 'conditional_embed', 128)
        self.conditional_hidden = misc.get_default(
            kwargs, 'conditional_hidden', [256, 256])
        self.const_lsig = misc.get_default(kwargs, 'const_lsig', True)
        self.mean_conditioning = misc.get_default(
            kwargs, 'mean_conditioning', False)
        self.do_vae = misc.get_default(kwargs, 'do_vae', False)
        self.set_transformations = misc.get_default(
            kwargs, 'set_transformations', [])
        self.levels = misc.get_default(kwargs, 'levels')
        self.seq_base = misc.get_default(kwargs, 'seq_base', True)
        # Corresponding model
        self.corr_irange = misc.get_default(kwargs, 'corr_irange', 1e-8)
        self.corr_use_scale = misc.get_default(kwargs, 'corr_use_scale', True)
        self.corr_scale_function = misc.get_default(
            kwargs, 'corr_scale_function')
        self.num_corr_trans = misc.get_default(kwargs, 'num_corr_trans')
        self.corr_hidden_sizes = misc.get_default(
            kwargs, 'corr_hidden_sizes', [])
        self.embed_corr_model = misc.get_default(
            kwargs, 'embed_corr_model', False)

        self.iid = misc.get_default(kwargs, 'iid', False)
        self.get_forward = misc.get_default(kwargs, 'get_forward', False)
        self.grow_conditional = misc.get_default(
            kwargs, 'grow_conditional', False)

...
class TANModel(Model):
    ...
    def __init__(self, transformations, conditional_model,
        ...
        Args:
            ...
            conditional_model: autoregressive conditional model function that
                ...
                conditioning values.
            likefunc: function to give the d conditional likelihoods
    ...
    def build_graph(self, inputs, conditioning=None, sampler_conditioning=None,
        ...
        # Flatten conditioning values.
        if conditioning is not None:
            cond_shape = conditioning.get_shape().as_list()[1:]
            conditioning = tf.reshape(
                conditioning, (-1, np.prod(cond_shape)), 'cond_flat')
        ...
        if sampler_conditioning is None:
            ...
            sampler_conditioning = tf.reshape(
                sampler_conditioning, (-1, np.prod(cond_shape)), 'cond_flat')

        # Do transformation on input variables.
        # TODO: Regularizer option
        with tf.variable_scope(
            'transformations',
            # regularizer=tf.contrib.layers.l2_regularizer(1.0, 'l2_t_reg'),
        ) as trans_scope:
        ...
        with tf.variable_scope(
            ...
            if sampler_conditioning is not None:
                self.sample_size = tf.shape(sampler_conditioning)[0]

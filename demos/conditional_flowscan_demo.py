...
def main(
    ...
    # When using conditioning information, the lambda functions muust take two
    # arguments!
    pnt_transformations = [
        lambda x, cond: trans.linear_map(x),
        lambda x, cond: trans.conditioning_transformation(
            x, cond, code_nvp_hidden, 1e-6, 1e-6, use_scale=use_scale,
            scale_function=scale_function),
        lambda x, cond: trans.leaky_transformation(x),
        lambda x, cond: trans.rnn_coupling(x, conf.rnn_coupling_class),
        lambda x, cond: trans.log_rescale(x),
        lambda x, cond: trans.leaky_transformation(x),
        lambda x, cond: trans.reverse(x),
        lambda x, cond: trans.conditioning_transformation(
            x, cond, code_nvp_hidden, 1e-6, 1e-6, use_scale=use_scale,
            scale_function=scale_function),
        lambda x, cond: trans.rnn_coupling(x, conf.rnn_coupling_class),
    ...
    # The point by point transformation should be taking a cond argument!
    set_pnt_transformations = [
        lambda X, c: trans.set_pnt_trans(
            X,
            lambda x, cond: trans.transformer(x, pnt_transformations, cond),
            c),
    ]
    set_transformations = [
        lambda X, c: trans.set_nvp(
            X, [], 2, 1e-6, embed_size=set_nvp_embed, use_scale=use_scale,
            scale_function=scale_function, conditioning_set_features=c,
        ),
        lambda X, c: trans.set_nvp(
    ...
    if use_set_transformations:
        # TODO: will only work for d=3 as coded
        transformations += set_transformations
        # One final conditional transformation
        # transformations += [
        #     lambda X, c: trans.set_pnt_trans(
        #         X,
        #         lambda x, cond: trans.conditioning_transformation(
        #             x, cond, code_nvp_hidden, 1e-6, 1e-6,
        #             scale_function=scale_function, use_scale=use_scale,
        #         ),
        #         c),
        # ]
